...
form.addEventListener('submit', async (e) => {
 ...
 const resp = await fetch(`https://allorigins.win{encodeURIComponent(targetUrl)}`);
 
 if (!resp.ok) throw new Error("No se pudo conectar con el servidor.");
 
 const dataJSON = await resp.json();
 const htmlTexto = dataJSON.contents;
 ...
 resultadosDiv.innerHTML = `
 <button class="btn-volver" onclick="volverAlInicio()">← Volver</button>
 <p style="margin-top:15px; text-align:center;">No se encontraron resultados en la web para tu búsqueda.</p>
 `;
 return;
 }
     resultadosDiv.innerHTML = `
     ...
     resultadosDiv.innerHTML = `
       <button class="btn-volver" onclick="volverAlInicio()">← Volver</button>
       <p style="margin-top:15px; color:#ff4d4d;">Error al conectar con la red de DuckDuckGo: ${err.message}</p>
     `;
   }
 }
});
