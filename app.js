...
// --- LÓGICA DEL HISTORIAL (Movido arriba para evitar ReferenceError) ---
...
// Recuerda la pantalla anterior para regresar //
...
form.addEventListener('submit', async (e) => {
 ...
 // MOTOR: WIKIPEDIA (RENDERIZADO INTERNO)
 if (engine === 'wikipedia') {
 home.classList.add('hidden');
 resultadosDiv.classList.remove('hidden');
 resultadosDiv.innerHTML = `<p class="loading-text">Buscando <b>${q}</b> en Wikipedia </p>`;
 try {
 ...
 <p style="margin-top:15px; text-align:center;">No se encontraron resultados</p>
 `;
 return;
 }
 resultadosDiv.innerHTML = `
 ...
 // === MOTOR: DUCKDUCKGO (TARJETAS NATIVAS) ===
 if (engine === 'duckduckgo') {
  ...
  resultadosDiv.innerHTML = `<p class="loading-text">Buscando <b>${q}</b> en la web con DuckDuckGo...</p>`;
    ...
    try {
    // IMPORTANTE: DuckDuckGo necesita obligatoriamente la subpágina /html/ y el parámetro ?q=
    const targetUrl = `https://duckduckgo.com{encodeURIComponent(q)}`;
    
    // Conectamos usando el proxy con el formato de URL correcto (? antes del target)
    const resp = await fetch(`https://corsproxy.io{encodeURIComponent(targetUrl)}`);
    
    if (!resp.ok) throw new Error("No se pudo conectar con el servidor proxy.");
    
    // CORRECCIÓN: corsproxy.io devuelve el código HTML directamente como texto, 
    // no metido dentro de un JSON con .contents como hacía allorigins.
    const htmlTexto = await resp.text();
    
    const parser = new DOMParser();
    const docVirtual = parser.parseFromString(htmlTexto, "text/html");
    const bloquesResultados = docVirtual.querySelectorAll('.result');
    
    if (bloquesResultados.length === 0) {
      resultadosDiv.innerHTML = `
        <button class="btn-volver" onclick="volverAlInicio()">← Volver</button>
        <p style="margin-top:15px; text-align:center;">No se encontraron resultados en la web.</p>
      `;
      return;
    }
    ...
    bloquesResultados.forEach(bloque => {
      const elementoTitulo = block || bloque.querySelector('.result__title');
      const elementoSnippet = bloque.querySelector('.result__snippet');
      const linkReal = bloque.querySelector('.result__url');

      if (elementoTitulo) {
        const tituloLimpio = elementoTitulo.textContent.trim();
        const urlReal = linkReal ? linkReal.getAttribute('href') : bloque.querySelector('a')?.href;
        const snippetLimpio = elementoSnippet ? elementoSnippet.textContent.trim() : "Sin descripción disponible.";
        ...
        tarjeta.innerHTML = `
          <div class="result-body" style="flex:1;">
            <a href="${urlReal}" target="_blank" style="font-weight:bold; font-size:18px; color:var(--link-color); text-decoration:none;">${tituloLimpio}</a>
            <p style="color:var(--text-secondary); font-size:14px; line-height:1.5; margin:5px 0 0 0;">${snippetLimpio}</p>
            <span style="font-size:11px; color:#4caf50; display:block; margin-top:6px; word-break:break-all; opacity:0.6;">${urlReal}</span>
          </div>
        `;
        resultadosDiv.appendChild(tarjeta);
      }
    ...
    resultadosDiv.innerHTML = `
      <button class="btn-volver" onclick="volverAlInicio()">← Volver</button>
      <p style="margin-top:15px; color:#ff4d4d;">Error al conectar con DuckDuckGo: ${err.message}</p>
    `;
  }
